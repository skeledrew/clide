...
class ClideConsole(GridLayout):
    ...
    def __init__(self, **kwargs):
        ...
        super(ClideConsole, self).__init__(**kwargs)
        # initialisations
    ...
    def on_enter(self, *args):
        ...
        def run_cmd(*args):
            ...
            if len(cmd) > 0:
                ...
                try:
                    add_to_cache(eval_expr(read_expr(cmd)))

                except Exception as e:
                    add_to_cache(''.join((str(err), ' <', cmd, ' >\n')))
...
class std_in_out(object):
    ...
    def read(self, no_of_bytes=0):
        if self.mode == 'stdin':
            # stdin.read
            Logger.exception('ClideConsole: can not read from a stdin pipe')
            return
